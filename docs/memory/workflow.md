...
0. **Session bootstrap** (first message only) — call `get_context(project="homelab", contributor="ethan", depth="standard")` via MCP. This returns active doctrine, open proposals, recent changes, and architecture in one call. Then ask one targeted question based on what the bundle surfaces — skip if intent is already unambiguous.
