...
0. **Session bootstrap** (first message only) — Claude calls `start_session(project)` and `get_context(project)` to hydrate from the ContextBundle (active doctrine, recent changes, active workstreams, current focus). If `core_operational_state.current_focus` is set, proceed directly. If empty, ask one targeted question to establish focus, then call `update_focus`. No greeting trigger — bootstrap fires on the first user message.
...
- Full constraints live in ~/canon/prismo/docs/context/constraints.md — read them
